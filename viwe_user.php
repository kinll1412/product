...
?>
  ...
  <!--               <div class="card-header py-3">
        ...
        foreach ($query as $data) {
            ...
            <td><center><?=$data['user_phone']?></center></td>
            <td><center><?=$data['status']?></center></td>            
            <td><center>
            <a href="edit_user.php?user_id=<?=$data['user_id']?>" class="btn btn-outline-dark"><span class="fas fa-user-edit"></span></a>
            <a href="del_user.php?user_id=<?=$data['user_id']?>" onclick="return confirm('คุณต้องการลบข้อมูลครุภัณฑ์ รหัส <?=$data['user_id']?> - <?=$data['user_name']?>')" class="btn btn-outline-danger"><span class="fa fa-trash"></span></a>
            </center></td>  
            </tr>
        <?php } ?>
        </tbody>
